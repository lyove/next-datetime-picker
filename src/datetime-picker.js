...
(function () {
  DateTimePicker = function (element, options, cb) {
    ...
    if (typeof options.ranges === "object") {
      ...
      for (let i = 0; i < rangesKeys.length; ++i) {
        ...
        txtelem.innerHTML = range;
        var rangeHtml = txtelem.value;
    ...
    utils.addClass(applyBtnEl, this.buttonClasses);
    utils.addClass(cancelBtnEl, this.buttonClasses);
    if (this.applyButtonClasses.length) {
      utils.addClass(applyBtnEl, this.applyButtonClasses);
    }
    if (this.cancelButtonClasses.length) {
      utils.addClass(cancelBtnEl, this.cancelButtonClasses);
    }
    utils.html(applyBtnEl, this.locale.applyLabel);
    utils.html(cancelBtnEl, this.locale.cancelLabel);
    ...
    /*
        ...
        this.container.find('.drp-calendar')
            .on('click.datetimepicker', '.prev', $.proxy(this.clickPrev, this))
            .on('click.datetimepicker', '.next', $.proxy(this.clickNext, this))
            .on('mousedown.datetimepicker', 'td.available', $.proxy(this.clickDate, this))
            .on('mouseenter.datetimepicker', 'td.available', $.proxy(this.hoverDate, this))
            .on('change.datetimepicker', 'select.yearselect', $.proxy(this.monthOrYearChanged, this))
            .on('change.datetimepicker', 'select.monthselect', $.proxy(this.monthOrYearChanged, this))
            .on('change.datetimepicker', 'select.hourselect,select.minuteselect,select.secondselect,select.ampmselect', $.proxy(this.timeChanged, this));
        --------------------------------------------------------------------------------------
        */
    ...
    utils.on(drpButtonsEl, "click", "button.applyBtn", this.clickApplyProxy);
    this.clickCancelProxy = function (e) {
      this.clickCancel(e);
    }.bind(this);
    utils.on(drpButtonsEl, "click", "button.cancelBtn", this.clickCancelProxy);

    if (this.element.tagName === "INPUT" || this.element.tagName === "BUTTON") {
      this.showProxy = function (e) {
        this.show(e);
      }.bind(this);
      utils.on(this.element, "click", this.showProxy);
      utils.on(this.element, "focus", this.showProxy);
      this.elementChangedProxy = function (e) {
        this.elementChanged(e);
      }.bind(this);
      utils.on(this.element, "keyup", this.elementChangedProxy);
      this.keydownProxy = function (e) {
        this.keydown(e);
      }.bind(this); //IE 11 compatibility
      utils.on(this.element, "keydown", this.keydownProxy);
    } else {
      this.toggleProxy = function (e) {
        this.toggle(e);
      }.bind(this);
      utils.on(this.element, "click", this.toggleProxy);
      utils.on(this.element, "keydown", this.toggleProxy);
    }
  ...
  DateTimePicker.prototype = {
    ...
    updateCalendars: function () {
      ...
      utils.off(drpCalendarElList, "click", ".prev", this.clickPrevProxy);
      utils.off(drpCalendarElList, "click", ".next", this.clickNextProxy);
      utils.off(
        drpCalendarElList,
        "mousedown",
        "td.available",
      ...
      if (this.timePicker) {
        ...
        if (this.endDate) {
          ...
          if (isNaN(minute)) {
            minute = parseInt(
              utils.findLast(
                this.container.querySelector(".left .minuteselect")
              ).value,
              10
            );
          }
          ...
          if (isNaN(minute)) {
            minute = parseInt(
              utils.findLast(
                this.container.querySelector(".right .minuteselect")
              ).value,
              10
            );
          }
      ...
      /*
            ...
            --------------------------------------------------------------------------------------
            */
      utils.on(drpCalendarElList, "click", ".prev", this.clickPrevProxy);
      utils.on(drpCalendarElList, "click", ".next", this.clickNextProxy);
      utils.on(
        drpCalendarElList,
        "mousedown",
        "td.available",
    ...
    renderTimePicker: function (side) {
      ...
      if (side == "left") {
        ...
        let timeSelector = this.container.querySelector(
        ...
        if (timeSelector.innerHTML != "") {
          selected.hour(
            !isNaN(selected.hour())
              ? selected.hour()
              : utils.findSelectedOption(
                  timeSelector.querySelector(".hourselect")
                ).value
          );
          selected.minute(
          ...
          if (!this.timePicker24Hour) {
            let ampm = utils.findSelectedOption(
    ...
    move: function () {
      ...
      if (!(this.parentEl.tagName === "BODY")) {
        let parentElOffset = utils.offset(this.parentEl);
        parentOffset = {
      ...
      let elementOffset = utils.offset(this.element);
      switch (drops) {
        case "auto":
          containerTop =
    ...
    show: function (e) {
      ...
      this.element.dispatchEvent(
        new CustomEvent("show.datetimepicker", { bubbles: true, detail: this })
      );
      this.isShowing = true;
    },
    ...
    hide: function (e) {
      ...
      this.container.style.display = "none";
      this.element.dispatchEvent(
        new CustomEvent("hide.datetimepicker", { bubbles: true, detail: this })
      );
      this.isShowing = false;
    ...
    outsideClick: function (e) {
      var target = e.target;
      // if the page is clicked anywhere except within the datetimepicker/button
      // itself then call this.hide()
      if (
        // ie modal dialog fix
        e.type == "focusin" ||
        target.closest(utils.getSelectorFromElement(this.element)) ||
        target.closest(utils.getSelectorFromElement(this.container)) ||
      ...
      this.hide();
      this.element.dispatchEvent(
        new CustomEvent("outsideClick.datetimepicker", {
          bubbles: true,
          detail: this,
        })
      );
    ...
    showCalendars: function () {
      ...
      this.move();
      this.element.dispatchEvent(
        new CustomEvent("showCalendar.datetimepicker", {
          bubbles: true,
          detail: this,
        })
      );
    ...
    hideCalendars: function () {
      ...
      this.element.dispatchEvent(
        new CustomEvent("hideCalendar.datetimepicker", {
          bubbles: true,
          detail: this,
        })
      );
    ...
    hoverDate: function (e) {
      ...
      if (!this.endDate) {
        ...
        for (let i = 0; i < tdElList.length; ++i) {
          ...
          let dt = cal.classList.contains("left")
    ...
    calculateChosenLabel: function () {
      ...
      if (customRange) {
        if (this.showCustomRangeLabel) {
          let rangesLiLastEl = utils.findLast(
    ...
    clickApply: function (e) {
      ...
      e.target.dispatchEvent(
        new CustomEvent("apply.datetimepicker", {
          bubbles: true,
          detail: this,
        })
      );
    ...
    clickCancel: function (e) {
      ...
      this.hide();
      e.target.dispatchEvent(
        new CustomEvent("cancel.datetimepicker", {
          bubbles: true,
          detail: this,
        })
      );
    ...
    timeChanged: function (e) {
      ...
      if (isNaN(minute)) {
        minute = parseInt(
          utils.findLast(cal.querySelectorAll(".minuteselect option")).value,
          10
        );
      }
      let second = this.timePickerSeconds
    ...
    remove: function () {
      ...
      if (
        ...
        if (this.showProxy) {
          utils.off(this.element, "click", this.showProxy);
          utils.off(this.element, "focus", this.showProxy);
          delete this.showProxy;
        }
        if (this.elementChangedProxy) {
          utils.off(this.element, "keyup", this.elementChangedProxy);
          delete this.elementChangedProxy;
        }
        if (this.keydownProxy) {
          utils.off(this.element, "keydown", this.keydownProxy);
          delete this.keydownProxy;
        }
      } else {
        if (this.toggleProxy) {
          utils.off(this.element, "click", this.toggleProxy);
          utils.off(this.element, "keydown", this.toggleProxy);
  ...
  const utils = {
    ...
    addClass: function (el, classes) {
      ...
      if (typeof el.length === "number") {
        for (let i = 0; i < el.length; ++i) {
          utils.addClassSub(el[i], classes);
        }
      } else {
        utils.addClassSub(el, classes);
      }
    },
    ...
    off: function (el, event, param1, param2) {
      ...
      if (typeof param1 === "function") {
        // param is listener
        utils.offSub(el, event, param1);
      } else {
        // param is selector
        if (typeof el.length === "number") {
          for (let i = 0; i < el.length; ++i) {
            utils.offSub(el[i].querySelectorAll(param1), event, param2);
          }
        } else {
          utils.offSub(el.querySelectorAll(param1), event, param2);
        }
      }
    },
    ...
    on: function (el, event, param1, param2) {
      ...
      if (typeof param1 === "function") {
        // param1 is listener
        utils.onSub(el, event, param1);
      } else {
        // param1 is selector
        if (typeof el.length === "number") {
          for (let i = 0; i < el.length; ++i) {
            utils.onSub(el[i].querySelectorAll(param1), event, param2);
          }
        } else {
          utils.onSub(el.querySelectorAll(param1), event, param2);
        }
      }
    },
  };
